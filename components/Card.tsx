...
const Card = ({ title, description, imageUrl, skills, githubUrl, videoUrl }: { title: string, description: string, imageUrl: string, skills: string[], githubUrl: string, videoUrl: string }) => {
  return (
    <div className="card-container flex relative">
      <div className="bg-white shadow-lg rounded-lg overflow-hidden max-w-md">
        <Image className="w-full" width={300} height={700} src={imageUrl} alt={title} />
        <div className="px-6 py-4">
          <h3 className="text-xl font-semibold mb-2">{title}</h3>
          <p className="text-gray-700 text-base mb-4">{description} <a href={videoUrl} target="_blank" rel="noopener noreferrer" className="text-blue-700"> Learn more </a></p>
